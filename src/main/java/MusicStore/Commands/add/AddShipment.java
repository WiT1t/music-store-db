...
public class AddShipment implements Command {
    ...
    public void execute(Connection connection, Scanner scanner) {
        try{
            ...
            addShipmentInDatabase(connection, now, staffID, prodID, quantity);
        }
        catch (Exception e) {
    ...
    private void addShipmentInDatabase(Connection connection, LocalDateTime now, int staffID, int product, int quantity) {
        String sql = "{CALL AddShipment(?,?,?,?)}";
        try(CallableStatement callableStatement = connection.prepareCall(sql)) {
            ...
            System.out.println("Successfully added a new shipment.");
        }
        catch(Exception e) {
            e.printStackTrace();
        }
